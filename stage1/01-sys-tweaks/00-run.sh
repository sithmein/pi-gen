...
if [ "${FILE_SYSTEM_TYPE}" == "ext4" ]; then
	...
	echo "ROOTDEV  /               btrfs   subvol=@,compress=zstd,defaults,noatime,ssd_spread  0       1" >> "${ROOTFS_DIR}/etc/fstab"
	echo "ROOTDEV  /home           btrfs   subvol=@home,compress=zstd,defaults,noatime,ssd_spread  0       1" >> "${ROOTFS_DIR}/etc/fstab"
else
